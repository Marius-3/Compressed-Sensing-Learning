...
该步骤与 MP 使用相同的最大相关准则，但 OMP 只在尚未进入支撑集的原子中进行选择，以避免重复选择同一原子。
...
OMP 的残差更新可以直观理解为：从观测向量中减去所有已选原子的联合拟合结果。
